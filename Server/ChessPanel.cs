...
namespace Server
{
    public partial class ChessPanel : UserControl
    {
        ...
        public void ChessPanel_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            if (isGameOver  || !mColor.Equals(mWhoseTurn) || APieces == null)
            {
                return;  //
            }
            ...
            {
               Debug.WriteLine(APieces[0].ToString());
                Score score = new Score(APieces, new Chess(0,0,AColorQ[0]), new Chess(0,0,BColorQ[0]));
                List<int>aiPos=score.getPosition();
                Chess aiChess = new Chess(aiPos[0], aiPos[1], colors.IndexOf(BCurrentColors[0]));
                x = (int)((aiPos[0] + ratioPieceOfLineHeight / 4) * mLineLength) + mOffset;
                y = (int)((aiPos[1] + ratioPieceOfLineHeight / 4) * mLineLength);
                //width = (int)(mLineLength * ratioPieceOfLineHeight);
